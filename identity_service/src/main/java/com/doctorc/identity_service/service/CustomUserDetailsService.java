...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if (patient.isPresent()) return new CustomUserDetails(patient.get().getEmail(), patient.get().getPassword());

        Optional<Admin> admin = adminRepository.findByEmail(username);
        if (admin.isPresent()) return new CustomUserDetails(admin.get().getEmail(), admin.get().getPassword());

        Optional<Dentist> dentist = dentistRepository.findByEmail(username);
        if (dentist.isPresent()) return new CustomUserDetails(dentist.get().getEmail(), dentist.get().getPassword());
